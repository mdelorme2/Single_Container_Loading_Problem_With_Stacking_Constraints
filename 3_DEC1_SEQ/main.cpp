	...
	try{
		...
		for (int i = 0; i < cuts.size(); i++){
			...
			for(int j = 0; j < cuts[i].size(); j++){
				RHS++;
				LHS += isItemCSPPackedV[cuts[i][j][0]][cuts[i][j][1]];
			}
		...
		model.addConstr(objFun == allo.infos.LB);
		model.setObjective(totArea, GRB_MINIMIZE);;
		model.optimize();		
