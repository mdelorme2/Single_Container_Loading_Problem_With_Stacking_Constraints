...
int model(Allocation& allo){
	...
	try{
		...
		model.optimize();
		allo.infos.nbCuts1 = cb.getNcuts1();
		allo.infos.nbCuts2 = cb.getNcuts2();
...
mycallbackD1::mycallbackD1(const Allocation& xallo, const vector<GRBVar>& xisItemPackedV, const vector<vector<GRBVar> >& xisItemCSPPackedV) {
    ...
    this->ncuts1 = 0;  						// the number of cuts, initially 0
	this->maxV = 0;
    this->allo = xallo;     				// the problem
	this->allo.infos.nbCuts2 = 0;
    this->isItemPackedV = xisItemPackedV;  	// the knapsack solution
	this->isItemCSPPackedV = xisItemCSPPackedV;
	this->bestSol = -1;
}

int mycallbackD1::getNcuts1() {
    return this->ncuts1; // return the number of cuts
}

int mycallbackD1::getNcuts2() {
    return this->allo.infos.nbCuts2; // return the number of cuts
}
...
int step2(Allocation& allo){
	...
	try{
		...
		for (int i = 0; i < itemsCSP.size(); i++){
			...
			for (int j = 0; j < ceil(log2(demandCSP[i]))+1; j++){
				isItemCSPPackedInCoN[i][j].resize(W);
				isItemCSPPackedInCoR[i][j].resize(W);
				for (int k = 0; k < W; k++){
					if(NPW[k] && k + itemsCSP[i].wid <= W){
						if(k + itemsCSP[i].wid != W && k + itemsCSP[i].wid + minW > W && NPW[W - itemsCSP[i].wid])
					...
					if(NPW[k] && k + itemsCSP[i].widR <= W){
						if(itemsCSP[i].lenR == itemsCSP[i].len && itemsCSP[i].widR == itemsCSP[i].wid)
							...
							if(k + itemsCSP[i].widR != W && k + itemsCSP[i].widR + minW > W && NPW[W - itemsCSP[i].widR])
								isItemCSPPackedInCoR[i][j][k] =  model.addVar(0, 0, 0, GRB_BINARY);
							else
		...
		for (int i = 0; i < W; i++){
			if(NPW[i])
				model.addConstr(columnFilled[i] <= L);
		}
		...
		for (int i = 0; i < itemsCSP.size(); i++){	
			for (int j = 0; j < ceil(log2(demandCSP[i]))+1; j++){
				for (int k = 0; k < W; k++){
					if(NPW[k] && k + itemsCSP[i].wid <= W && ceil(isItemCSPPackedInCoN[i][j][k].get(GRB_DoubleAttr_X) - EPSILON) == 1){
						for (int l = 0; l < pow(2,j); l++){
							int index = indexCSP[i].back();
							// get ordinates
							for(int m = 0; m < ordinates[i].size();m++){							
								if(ordinates[i][m][1] == k && ordinates[i][m][2] == 0){
									allo.pps[itemsBottom[index].idx].push_back(ordinates[i][m][0]);
									ordinates[i][m][2] = -1;
									break;
								}
							}
			...
			for (int j = 0; j < ceil(log2(demandCSP[i]))+1; j++){
				for (int k = 0; k < W; k++){
					if(NPW[k] && k + itemsCSP[i].widR <= W && ceil(isItemCSPPackedInCoR[i][j][k].get(GRB_DoubleAttr_X) - EPSILON) == 1){
						for (int l = 0; l < pow(2,j); l++){
							int index = indexCSP[i].back();	
							// get ordinates
							for(int m = 0; m < ordinates[i].size();m++){
								if(ordinates[i][m][1] == k && ordinates[i][m][2] == 1){
									allo.pps[itemsBottom[index].idx].push_back(ordinates[i][m][0]);
									ordinates[i][m][2] = -1;
									break;
								}
		...
		for(int i = 0; i < allo.pps.size();i++){
			for(int j = 0; j < allo.pps[i].size();j++){
				cout << allo.pps[i][j] << " ";
			}
			cout << endl;
		}
...
void mycallbackYC::callback() {
	if (where == GRB_CB_MIPSOL) {   	
		double initCPTimeModelCPU = getCPUTime();
		...
		try{
			...
			// If infeasible
			bool check = cp.solve();
			cout << " Find a solution in " << getCPUTime() - initCPTimeModelCPU << endl;
			if (!check){
