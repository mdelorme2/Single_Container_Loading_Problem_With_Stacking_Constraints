...
int main(int argc, char **argv){
	...
	vector<int> NPW;
	int ret1, ret2;
	step0(allo,itemsCSP,demandCSP,indexCSP,NPW);
	do{
		ret1 = step1(allo,cuts,itemsCSP,demandCSP,indexCSP,NPW);
		if(ret1 == 2) 
			break;
		ret2 = step2(allo,cuts.back(),itemsCSP,indexCSP);
	}while(ret2 == 1);
	cuts.pop_back(); allo.infos.nbCuts = cuts.size();
...
int step1(Allocation& allo, vector<vector<vector<int> > > & cuts, const vector<Item>& itemsCSP, const vector<int>& demandCSP, const vector<vector<int> >& indexCSP, const vector<int>& NPW){
	...
	int minW = allo.W;
	for(int i =0; i<indexCSP.size();i++){
		minW = min(minW,itemsCSP[i].wid); 
		minW = min(minW,itemsCSP[i].widR); 
	}
	...
	try{
		...
		for (int i = 0; i < cuts.size(); i++){
			...
			for(int j = 0; j < cuts[i].size(); j++){
				RHS++;
				if(cuts[i][j][0] == 0) LHS += isItemCSPPackedInCoN[cuts[i][j][1]][cuts[i][j][2]][cuts[i][j][3]];
				if(cuts[i][j][0] == 1) LHS += isItemCSPPackedInCoR[cuts[i][j][1]][cuts[i][j][2]][cuts[i][j][3]];
			}
		...
		// Callback
		mycallbackFCB cb = mycallbackFCB();
...
mycallbackFCB::mycallbackFCB() {
	cout << "Create the fake call back" << endl;
}

void mycallbackFCB::callback() {
    if (where == GRB_CB_MIPSOL) { 
		cout << "Enter the fake call back" << endl;
