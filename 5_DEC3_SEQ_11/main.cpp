...
int step0(Allocation& allo, vector<Item>& itemsCSP, vector<int>& demandCSP, vector<vector<int> >& indexCSP){
	...
	for(int i =0; i<allo.items.size();i++){
		...
		for(int j = 0; j<itemsCSP.size();j++){
			if(allo.items[i].len == itemsCSP[j].len && allo.items[i].wid == itemsCSP[j].wid && allo.items[i].lenR == itemsCSP[j].lenR && allo.items[i].widR == itemsCSP[j].widR && allo.items[i].col == itemsCSP[j].col){
				exist = true;
				demandCSP[j] += 1;
...
int step1(Allocation& allo, vector<vector<vector<int> > > & cuts1, const vector<Item>& itemsCSP, const vector<int>& demandCSP, const vector<vector<int> >& indexCSP){
	...
	try{
		...
		for (int i = 0; i < allo.items.size(); i++){
			for (int k = 0; k < allo.maxStack; k++){
				GRBLinExpr weightOnTop = 0;
				GRBLinExpr weightBottom = 0;
				for (int j = 0; j < allo.items.size(); j++){
					if(k > 0 && allo.cAdj[i][j]) weightBottom += isItemStackedInCo[i][j][k] * allo.items[j].wei;
					for (int l = k+1; l < allo.maxStack; l++){
						if(allo.cAdj[i][j]) weightOnTop += isItemStackedInCo[i][j][l] * allo.items[j].wei;
					}
				}
				if(k == 0) model.addConstr(weightOnTop <= allo.items[i].wei * isItemPackedV[i]);
				else model.addConstr(weightOnTop <= weightBottom);
			}
		}
...
int step3(Allocation& allo, const vector<vector<int> > & cut2, const vector<Item>& itemsCSP, const vector<vector<int> >& indexCSP){
	...
	try{
		...
		vector<vector<int> > taskIdxPerW (allo.W);
		for (int i = 0; i < allo.W; i++)
			...
			if (reducedInstance[i][2] == 0){
				...
				for (int j = 0; j < itemsCSP[reducedInstance[i][0]].wid; j++){
					if(isActivated[reducedInstance[i][1] + j]){
						...
						taskIdxPerW[reducedInstance[i][1] + j].push_back(i);
					}
				}
			}
			else {
		...
		for(int i = 0; i < allo.W;i++){
			if(isActivated[i]){
				...
				for(int j = 0; j < taskIdxPerW[i].size();j++){
					for(int k = 0; k < taskIdxPerW[i].size();k++){
						if(itemsCSP[reducedInstance[taskIdxPerW[i][j]][0]].col < itemsCSP[reducedInstance[taskIdxPerW[i][k]][0]].col){
							model.add(IloEndBeforeStart(env, Tasks[taskIdxPerW[i][j]], Tasks[taskIdxPerW[i][k]]));
						}							
					}
				}
			}
