...
void Item::print(){
	cout << idx << " " << id << " " << lenO << " " << widO << " " << heiO << " " << pro << " " << wei << " " << sta << " " << fla  << " [" << len << " " << wid  << " " << hei << "] [" << lenR << " " << widR << " " << hei << "]" << endl;
}
...
void Allocation::printInfo(const string& pathAndFileout){
	...
	file << infos.LB << "\t" << infos.UB <<  "\t"  << sumVol <<  "\t" << infos.nbVar << "\t" << infos.nbCons << "\t" << infos.nbNZ <<  "\t" << infos.nbCuts << "\t" << checkSolution() << endl;
	file.close();
}
